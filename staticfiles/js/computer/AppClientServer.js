...
function AppClientsocket(connectionType) {
  ...
  this.sendTransmitStack = function () {
   ...
   if (!AppClientConnection.socket.ajaxConnected) {
    ...
    params += "&data0=";
    AppClientConnection.socket.waitForNextConnectionTest = 10;
    if(AppClientConnection.socket.waitForNextConnectionTimer) 
      clearTimeout(AppClientConnection.socket.waitForNextConnectionTimer);
    AppClientConnection.socket.waitForNextConnectionTimer = setTimeout(function(){AppClientConnection.socket.sendTransmitStack();},500);
   }else{
   ...
   xmlHttpReq.open('POST', AppClientServerProtocol+"//live.learningapps.org/AJAXClient", true); 
   ...
   xmlHttpReq.onerror = function (){
      ...
      setTimeout(function(){AppClientConnection.socket.sendTransmitStack();},500);
   };
  ...
  this.activatePolling = function () {
    ...
    this.pollingRequest.open('POST', AppClientServerProtocol+"//live.learningapps.org/AJAXClientPoll", true); 
  ...
  this.windowUnload = function (){
   ...
   try{
     if(navigator.sendBeacon){
        navigator.sendBeacon(AppClientServerProtocol+"//live.learningapps.org/AJAXClient?uid="+
                             AppClientConnection.socket.uid+"&unload=1");
     }else{
       var r;
       if (window.XMLHttpRequest) r = new XMLHttpRequest();
       else if (window.ActiveXObject) r = new ActiveXObject("Microsoft.XMLHTTP");
       r.open('POST', AppClientServerProtocol+"//live.learningapps.org/AJAXClient", false);
...
*/
   try{
     ...
     lastTextToSpeechPlayer = new Audio(AppClientServerProtocol+"//learningapps.org/text-to-speech/t2s.php?tl="+lang+"&txt="+encodeURIComponent(s)+ "&rnd="+startUpTime);
